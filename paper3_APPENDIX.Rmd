...
In the tables below, I report the results of additional robust analyses testing the research hypotheses regarding the moderating role of perceived personal. In **Table A3** I add subjects' subjective reporting on the personal relevance of each of the two policy plans, and its interaction with the symbols and information manipulations, controlling for the areas. Higher scores of perceived personal relevance did not attentuate the positive effect of the real symbols. However, it did slightly increase the effect of the weak information, regarding the air-pollution policy, in line with Hypothesis 2.

In **Tables A4 - A6**, I examine the effect of the operationizations of personal relevance (i.e. the differences between the areas and the relevance manipulation) on three measures for subjects elaboration of the two policy plans. The first measure is the number of seconds participants spent on scanning the policy plans, log transformed (hereafter: reaction time). The second measure is participants' score in a memory test, which was conducted during the survey. Participants were asked to indicate whether a list of four items were included in the second policy plan which was presented to them, of which two were/were not included (participants could also select “I don’t know”). The memory score is calculated as the number of correct items selected by the participant minus the number of incorrect items, with a minimum score of zero. Accordingly, participants could receive the scores of 0, 1 or 2. The third measure is the delta of each subject’s trust in the strong and weak policy plans that were presented to them. The latter, within-subject analysis, utilizes the research design in which every participant was presented with one weak and one strong policies. Neither residing in the Haifa-bay nor the relevance treatment increase the elaboration of policy plans, based on these three measures.  

In **Tables A7** I add additional a set of individual-level control variables, and in **Table A8** I further control for additional factors that can be related to participants' motivation to elaborate, including having young children, general interest in environmental issues and following environmental issues in the media. The two final questions were presented only to those subjects assigned to the relevance treatment condition, and therefore, their analysis is based only on that sample (n = `r SVIVA2_01 %>% filter(RELEVANCE_exp==1) %>% nrow()`).Finally, in **Table A9**, I add those subjects who were filttered, except for those who are under the age of 18 (n_subjects = `r SVIVA2_comb_robust_1 %>%distinct(IP) %>% nrow()`). The effects of the symbols and information manipulations and their interaction with the areas and the relevance manipulation are consistent with the main models presented in the paper in all these analyses.       
